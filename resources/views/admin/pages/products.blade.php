...
@php
    $productTabs = [
        ['route' => 'all-products', 'label' => 'Все товары'],
        ['route' => 'peending-products', 'label' => 'На модерации'],
        ['route' => 'products-not-stock', 'label' => 'Нет в наличии'],
    ];
@endphp
...
<section class="space-y-6">
    <header class="rounded-3xl bg-white p-6 shadow-sm">
        <div class="flex flex-wrap items-center justify-between gap-4">
            <div>
                ...
                <p class="text-sm text-gray-500">Управляйте товарами, модерируйте заявки и держите актуальные остатки.</p>
            </div>
            <a href="{{ route('add-product') }}" class="inline-flex items-center rounded-2xl bg-indigo-600 px-5 py-2 text-sm font-semibold text-white shadow hover:bg-indigo-500">
                + Добавить товар
            </a>
        </div>
            ...
            @foreach ($productTabs as $tab)
                <a href="{{ route($tab['route']) }}"
                    @class([
                        ...
                        'bg-indigo-600 text-white shadow' => request()->routeIs($tab['route']),
    ...
    <form action="{{ route('update-products') }}" method="GET" class="space-y-6">
        ...
        <div class="rounded-3xl border border-gray-100 bg-white p-5 shadow-sm">
            <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                <div>
                    ...
                    <select name="category" class="mt-2 w-full rounded-2xl border border-gray-200 bg-gray-50 px-4 py-2 text-sm focus:border-indigo-500 focus:ring-indigo-500">
                ...
                <div>
                    ...
                    <select name="subcategory" class="mt-2 w-full rounded-2xl border border-gray-200 bg-gray-50 px-4 py-2 text-sm focus:border-indigo-500 focus:ring-indigo-500">
            ...
            <div class="mt-4 flex flex-wrap gap-3">
                <button type="submit" name="action" value="filter" class="inline-flex items-center rounded-2xl bg-indigo-600 px-5 py-2 text-sm font-semibold text-white shadow">
                    Применить фильтры
                </button>
                <a href="{{ route('all-products') }}" class="inline-flex items-center rounded-2xl bg-gray-100 px-5 py-2 text-sm font-semibold text-gray-600">
                    Сбросить
                </a>
            </div>
        </div>

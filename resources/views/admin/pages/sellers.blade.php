...
@section('content')
    ...
    <section class="space-y-6">
        ...
        <form method="GET" action="{{ $filterRoute }}"
            class="rounded-3xl border border-gray-100 bg-white p-6 shadow-sm space-y-4">
            ...
            <div class="flex flex-wrap gap-3">
                ...
                <a href="{{ route($activeTab === 'pending' ? 'peending-sellers' : 'sellers') }}"
                    class="inline-flex items-center rounded-2xl bg-gray-100 px-5 py-2 text-sm font-semibold text-gray-600">
                    Сбросить
                </a>
            </div>
        </form>
                            ...
                            <tr class="hover:bg-gray-50/70">
                                <td class="px-5 py-4">
                                    <div class="flex items-center gap-3">
                                        <div
                                            class="h-12 w-12 overflow-hidden rounded-2xl border border-gray-100 bg-gray-50">
                                            <img src="{{ $logo }}" alt="{{ $seller->store_name }}"
                                                class="h-full w-full object-cover">
                                        </div>
                                        <div>
                                            <p class="text-sm font-semibold text-gray-900">{{ $seller->store_name }}</p>
                                            <p class="text-xs text-gray-500">{{ $seller->store_phone }}</p>
                                            <span
                                                class="mt-1 inline-flex items-center rounded-full px-2.5 py-0.5 text-[11px] font-semibold ring-1 {{ $verifiedBadge }}">
                                ...
                                <td class="px-5 py-4 text-sm font-semibold text-gray-900">
                                    {{ $seller->products->sum('stock') }} шт.</td>
                                <td class="px-5 py-4 text-sm font-semibold text-gray-900">
                                    {{ $seller->products->sum('sell') ?? 0 }} шт.</td>
                                <td class="px-5 py-4">
                                    <span
                                ...
                                <td class="px-5 py-4 text-right">
                                    <div class="flex items-center justify-end gap-2">
                                        ...
                                        <form action="{{ route('delete-seller', $seller->id) }}" method="POST"
                                            onsubmit="return confirm('Удалить продавца {{ $seller->store_name }}? Это действие необратимо.');">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"

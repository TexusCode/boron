...
@section('content')
    <div class="whatsapp-button-wrapper">
        <a href="https://wa.me/+992100604040?text=Салом! Артикул: {{ $product->code }}. Я заинтересован на этот товар https://boron.tj/details/{{ $product->id }}"
            class="whatsapp-button" aria-label="Chat on WhatsApp">
            ...
            <svg viewBox="0 0 16 16" class="whatsapp-icon" fill="currentColor" height="24" width="24"
        ...
        <div class="max-w-screen-xl px-2 mx-auto 2xl:px-0">
            <div class="lg:flex lg:items-start lg:gap-8">
                    ...
                    <div id="controls-carousel" class="relative w-full" data-carousel="static">
                        <div class="overflow-hidden swiper swiper_main rounded-xl">
                            <div class="swiper-wrapper" id="lightgallery">
                                ...
                                <a href="{{ asset('storage/' . $product->miniature) }}"
                                    class="w-full h-full bg-black swiper-slide aspect-h-5 lg:aspect-h-4 aspect-w-5">

                                    <img src="{{ asset('storage/' . $product->miniature) }}"
                                        class="object-cover object-center w-full h-full thumbnail" loading="lazy">
                ...
                <div class="mt-6 space-y-4 md:min-w-96 sm:mt-8 lg:mt-0">
                    ...
                    @php
                        $percentOff = $product->total_discount_percent;
                        $displayPrice = $product->display_price;
                        $formattedDisplayPrice = rtrim(rtrim(number_format($displayPrice, 2, '.', ''), '0'), '.');
                    ...
                    <div class="flex items-start justify-between gap-4">
                        <div>
                            @if ($percentOff > 0)
                                <div class="flex items-center gap-2">
                                    <p class="text-xl font-bold text-gray-900">
                                        <strike> {{ $formattedOriginalPrice }}c </strike>
                                    </p>
                                    <span
                            ...
                            @endif
                            <p class="text-3xl font-extrabold text-gray-900">{{ $formattedDisplayPrice }}c</p>
                        </div>
                    ...
                    <div class="grid grid-cols-3 gap-2">
                        <label class="relative flex">
                            <div
                                class="relative flex-1 space-y-0.5 overflow-hidden rounded-lg border border-green-200 p-2 peer-checked:border-primary-700 ">
                                <p class="text-sm font-medium leading-none text-gray-900">
                                    {{ round($product->display_price / 3) }} c/3м
                                    <span class="font-bold text-green-500">0%</span>
                                </p>
                                <p class="text-xs font-normal text-gray-500 dark:text-gray-400">Рассрочка с картой <span
                                        class="text-red-500">"Салом"</span> от <a href=""
                                        class="font-bold text-green-400">Alif Bank</a></p>
                            </div>
                        </label>
                        <label class="relative flex">
                            <div
                                class="relative flex-1 space-y-0.5 overflow-hidden rounded-lg border border-green-200 p-2 peer-checked:border-primary-700 ">
                                <p class="text-sm font-medium leading-none text-gray-900">
                                    {{ round(($product->display_price * 1.1) / 6) }} c/6м
                        ...
                        <label class="relative flex">
                            <div
                                class="relative flex-1 space-y-0.5 overflow-hidden rounded-lg border border-green-200 p-2 peer-checked:border-primary-700 ">
                                <p class="text-sm font-medium leading-none text-gray-900">
                                    {{ round(($product->display_price * 1.2) / 12) }} c/12м
                                    <span class="font-bold text-green-500">20%</span>
                                </p>
                                <p class="text-xs font-normal text-gray-500 dark:text-gray-400">
                                    Рассрочка с картой <span class="text-red-500">"Салом"</span> от <a href=""
                                        class="font-bold text-green-400">Alif Bank</a></p>
                            </div>
                        </label>
                    </div>
                ...
                <div
                    class="grid grid-cols-1 gap-6 mt-6 md:min-w-72 shrink-0 sm:mt-8 sm:grid-cols-2 lg:mt-0 lg:w-48 lg:grid-cols-1 lg:pr-8">
                    ...
                    <div class="space-y-2">
                        <div class="p-2 space-y-1 border rounded-lg border-primary-200 bg-primary-50">
                            ...
                            @if ($product->seller->isverified)
                                <div
                                    class="flex items-center justify-center gap-2 py-1 mb-2 text-green-500 bg-green-100 border-2 border-green-300 rounded-md">
                            ...
                            <div class="grid grid-cols-4 gap-2">
                                <div class="p-2">
                                    <img src="{{ asset('storage/' . $product->seller->logo) }}" alt=""
                                        class="object-contain w-full h-full">
                                </div>
                                <div class="col-span-3">
                                    <h1 class="mb-2 font-bold leading-none uppercase line-clamp-1 text-primary-600">
                                        {{ $product->seller->store_name }}</h1>
                                    <div
                                        class="grid grid-cols-2 px-2 py-1 text-sm rounded-lg font-me bg-primary-100 text-primary-600">
                                ...
                                @endif
                                <a href="{{ route('page-seller', $product->seller->id) }}"
                                    class="flex justify-center p-2 font-semibold rounded-md bg-primary-100 text-primary-600 hover:underline lg:text-sm col-span-full">
        ...
        <section class="bg-white">
            <div class="max-w-screen-xl px-2 mx-auto mb-4">
                <div class="max-w-screen-xl text-gray-500 sm:text-lg dark:text-gray-400">
                    ...
                    <div class="product-description text-gray-800 dark:text-gray-100">
                        {!! $product->description !!}
                    </div>
                </div>
            </div>
...
@section('opengraph')
    ...
    <meta property="og:description"
        content="{{ \Illuminate\Support\Str::limit(strip_tags($product->description ?? ''), 160) }}">

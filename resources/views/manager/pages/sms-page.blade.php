...
<section class="space-y-6">
    ...
    <div class="rounded-3xl bg-white p-6 shadow-sm">
        ...
        <form method="GET" action="{{ route('manager.sms-page') }}" class="mt-5 flex flex-wrap items-end gap-3 text-sm">
            <div>
                ...
                <tbody class="divide-y divide-gray-100 bg-white">
                    @foreach ($clients as $client)
                        <tr class="hover:bg-gray-50/70">
                            ...
                            <td class="px-5 py-4">
                                <form action="{{ route('manager.sms-clients.toggle', $client) }}" method="POST">
                                    @csrf
                                    @method('PATCH')
                                    <button type="submit"
...
<div id="single-sms-modal" tabindex="-1" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/40 p-4">
    <div class="w-full max-w-lg rounded-3xl bg-white p-6 shadow-2xl">
        ...
        <form action="{{ route('manager.onesms') }}" method="POST" class="mt-4 space-y-4">
            @csrf
            <div>
...
<div id="bulk-sms-modal" tabindex="-1" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/40 p-4">
    <div class="w-full max-w-lg rounded-3xl bg-white p-6 shadow-2xl">
        ...
        <form action="{{ route('manager.sms-many') }}" method="POST" class="mt-4 space-y-4">
            @csrf
            <div>
                <label class="text-sm font-semibold text-gray-700">Шаблон</label>
                <select name="template_id"
...
<div id="client-modal" tabindex="-1" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/40 p-4">
    <div class="w-full max-w-lg rounded-3xl bg-white p-6 shadow-2xl">
        ...
        <form action="{{ route('manager.sms-clients.store') }}" method="POST" class="mt-4 space-y-4">
            @csrf
            <div>

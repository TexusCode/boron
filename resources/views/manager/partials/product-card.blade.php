...
<div class="group relative flex flex-col rounded-3xl border border-gray-100 bg-white p-3 text-sm shadow-sm transition hover:-translate-y-1 hover:shadow-xl">
    <div class="relative overflow-hidden rounded-2xl bg-gray-100">
        ...
        <div class="absolute bottom-3 right-3 flex gap-2">
            <a href="{{ route('details', $product->id) }}" class="inline-flex h-10 w-10 items-center justify-center rounded-2xl bg-white/90 text-gray-800 shadow hover:bg-emerald-600 hover:text-white">
    ...
    <div class="mt-4 flex flex-1 flex-col gap-2">
        <div>
            <a href="{{ route('details', $product->id) }}" class="line-clamp-2 text-base font-semibold text-gray-900 hover:text-emerald-600">
                {{ $product->name }}
            </a>
        ...
        <div class="text-xs text-gray-500">
            ...
            @endif
            <p>Магазин: <span class="font-semibold text-emerald-600">{{ $product->seller->store_name }}</span></p>
        </div>
